...
%function to integrate (x^2*sin(x)) from a,b
function integration(a,b)
    %integrate (x^2*sin(x)) 
    c=(2-b^2)*cos(b)+2*b*sin(b)-(2-a^2)*cos(a)+2*a*sin(a); %calculate the definite integral from a to b
    disp(['The result of the integration is: ',num2str(c)])  %display the result
   
end

integration(0,3*pi/2)    %call integration function with n=3*pi/2
...
match = zeros(1,length(c));              %initialize array match to store results of comparison
